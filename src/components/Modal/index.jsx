...
const Modal = ({ tweet, close }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    //güncellenicek olan dökümanın ref. al
    ...
    try {
      ...
      //dosya seçildiyse hem yazı hem fot. güncellle

      //Seçilen foto storage'a yükle
      const newUrl = await upload(file);
      // Belgenin hem yazı hem fot değerni güncelle
      await updateDoc(tweetRef, {
        textContent: text,
        imageContent: newUrl,
        isEdited: true,
      });
      // BaşarıBildirim gönder
      ...
      <div className="bg-black rounded-md py-10 px-8 w-3/4 max-w-[600px] min-h-[60vh] max-h-[80vh] flex flex-col">
        <div className="flex justify-between">
          ...
          <button onClick={close}>
            <MdClose className="text-3xl transition hover:text-gray-500" />
          </button>
        </div>
