...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isSignUp) {
      ...
      //Giriş yapma odundaysa: hesap giriş yap
      signInWithEmailAndPassword(auth, email, pass)
        .then(() => {
          toast.success("Hesaba gririş yapıldı");
          navigate("/feed");
        })
        .catch((err) => {
          //eğer giriş bilgilri yanlış hatas geldiyse;
          if (err.code === "auth/invalid-credential") {
            //error state ini true çek
            setIsError(true);
          }
    ...
    <div className="h-screen bg-[#242424] text-white grid place-items-center">
      <div className="bg-black flex flex-col gap-10 py-16 px-32 rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <input
            className="text-black rounded mt-1 p-2 outline-none shadow-lg focus:shadow-[gray]"
            type="password" // Şifre girişi için type="password" kullanımı
            required
        ...
        <p className="mt-5">
          ...
          <span
            onClick={() => setSignUp(!isSignUp)} // Düzgün şekilde toggle
